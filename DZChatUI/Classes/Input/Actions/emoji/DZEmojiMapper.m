...
int const ConstEmoji[] =
    {0x1f60a, 0x1f60c, 0x1f61a, 0x1f613, 0x1f630, 0x1f61d, 0x1f601, 0x1f61c, 0x263a,0x1f609,
    ...
    0x1f3e8, 0x1f3e7, 0x1f3ea, 0x1f6b9, 0x1f6ba, 0xa9, 0xae, 0x2122, 0x263a, 0x26a0,
    ...
    0x1f621, 0x1f603, 0x1f604, 0x1f605, 0x1f606, 0x1f607, 0x1f608, 0x1f60a, 0x1f60b, 0x1f60d,
    0x1f60e, 0x1f610, 0x1f611, 0x1f613, 0x1f615, 0x1f617, 0x1f619, 0x1f61b, 0x1f61e, 0x1f61f,
...
- (instancetype) init
{
    ...
    UIImage* defaultImage = DZCachedImageByName(@"emoji_fallback");
    
    int totalMaxEmojiCount = 258;
    int index = 0;
    NSMutableDictionary* keyMaps = [NSMutableDictionary new];
    int count = sizeof(ConstEmoji)/sizeof(int);
    for (int i = 0 ; i < count; i++) {
        UIImage* image;
        NSString *originEmoji = [[NSString alloc] initWithBytes:&ConstEmoji[i] length:4 encoding:NSUTF32LittleEndianStringEncoding];
        for (; index <= totalMaxEmojiCount ; ) {
            NSString* key = [NSString stringWithFormat:@"emoji_%d%d%d",(index/100)%10,(index/10)%10,index%10];
            ...
            NSLog(@"表情Null%d", index);
            index++;
        }
        if (image) {
            keyMaps[@(index)] = originEmoji;
            if (_mapper[originEmoji]) {
                NSLog(@"!!!!!! exist %@  %d",originEmoji, ConstEmoji[i]);
            }
